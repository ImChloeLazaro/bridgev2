...
const ThreeMedia = ({ data, orientation, layout }) => {
  const filteredPost =
    layout === "single" ? data.slice(0, 1) : data.slice(0, 3);
  ...
  const alignment = {
    landscape:
      "gap-1.5 grid grid-cols-1 grid-rows-3 bg-white-default/60 h-full px-0 lg:px-4",
    portrait:
      ...
      {filteredPost.map((media, index) => {
        return (
          <Button
            ...
            disableRipple
            disableAnimation
            className={`${featuredMedia} relative bg-grey-default/60 backdrop-blur-sm flex justify-center items-center overflow-clip h-full w-full p-0`}
            onPress={() => {
              handleOnOpen(index);
              onOpen();
            }}
