...
const URL = 'wss://ettpkpovgl.execute-api.ap-southeast-1.amazonaws.com/production/';
...
const Test = () => {
    ...
    useEffect(() => {
        ...
        socketRef.current.onopen = () => {
            ...
            fetchUserAttributes().then(attr => {
                setUser(attr);
                socketRef.current.send(JSON.stringify({ action: 'setdata', name: attr.name, email: attr.email, sub: attr.sub, picture: attr.picture }));
            })
        };
    ...
    const updateNotificationState = (data, action) => {
        ...
        socketRef.current.send(JSON.stringify({ action: 'notification', id: data, route }));
    ...
    const getNotificationId = (proxy, data) => {
        if (['read', 'unread', 'hide', 'setshow'].includes(proxy)) {
            ...
            const notificationToDelete = notifications.find(notification => notification._id === data && notification.unread);
            if (notificationToDelete) {
                // If the notification to be deleted is unread, decrease the notification count
                setNotificationCount(prevCount => prevCount - 1);
            }
            socketRef.current.send(JSON.stringify({ action: 'notification', id: data, route: 'delete' }));
            setNotifications(prevNotifications => prevNotifications.filter(notification => notification._id !== data));
        }
    };
    ...
    const sendnotification = () => {
        ...
        socketRef.current.send(JSON.stringify({ action: 'notification', subs, title, type, description, notified_from: user,  route: 'set' }));
    }
    return (
        <>
