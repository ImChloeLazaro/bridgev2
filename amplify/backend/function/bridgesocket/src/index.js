...
exports.handler = async (event) => {
    try {
        const ENDPOINT = 'https://ettpkpovgl.execute-api.ap-southeast-1.amazonaws.com/production/';
        ...
        if (event.requestContext) {
            ...
            switch (routeKey) {
                case '$connect':
                    break;
                case '$disconnect':
                    await sendMany(await session_ids(), { message: `${await session_name(connectionId)} disconnected` });
                    await UserSession.deleteOne({ id: connectionId });
                    await sendOne(connectionId, { message: `users length: ${(await session_ids()).length}` });
                    break;
                case '$default':
                    break;
                case 'setdata':
                    try {
                        ...
                        await UserSession.create({ id: connectionId, name, sub, email, picture });
                        await sendMany(await session_ids(), { system: `${body.name} joined the channel.` });

                        await sendOne(connectionId, { system: `Active users: ${(await session_ids()).length}` });
                        const notifications = await Notification.find({ sub })
                        await sendOne(connectionId, { notifications, count: await notification_count(sub)});
                ...
                case 'notification':
                    try {
                        ...
                        const { subs, title, type, description, route, notified_from } = body;
                        ...
                        if (route === 'set') {
                            ...
                            for (const session of temp_ids) {
                                const notificationsForSession = insertedNotifications.filter(notification => notification.sub === session.sub);
                                const count = await notification_count(session.sub);
                                await sendOne(session.id, { notifications: notificationsForSession, count });
                            }
                        }
                        ...
                        if (route === 'setread') {
                            ...
                            await sendOne(connectionId, { system: 'Marked as read id ' + id });
                        }
                        if (route === 'setunread') {
                            // Mark the notification as read
                            const { id } = body
                            await Notification.updateOne({ _id: id }, { unread: true });
                            await sendOne(connectionId, { system: 'Marked as unread id ' + id });
                        }
                        ...
                        await sendOne(connectionId, { system: error.message, success: false });
                    }
                    break;
                case 'chat':
